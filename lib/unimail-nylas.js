...
class NylasConnector extends EventEmitter {
     ...
     *
     * @returns {MessageResource | Object | String} Returns a unified message resource when options.raw is falsy or the raw response of the API when truthy
     */
    ...
    listMessages(auth, params, options, callback) {
        ...
        return nylas.messages.list(nylasParams).then((response) => {
            ...
            .catch((error) => {
                ...
                if (error.message) {
                    if (error.message.indexOf('Couldn\'t find') > -1) {
                        statusCode = 404;
                    }

                    if (error.message.indexOf('Too many concurrent query requests') > -1) {
                        statusCode = 429;
                    }
                }

                return callback(Boom.boomify(error, { statusCode: statusCode }));
            });
    }
