	...
	listMessages(auth, params, options, callback) {
		...
		if (clonedParams.after && clonedParams.participants && clonedParams.participants.length > 0) {
			if (clonedParams.participants.length > 1) {
				return Async.map(clonedParams.participants, (participant, callback) => {
					...
					return Async.reduce(results, { messages: [], next_page_token: 0 }, (responseObject, result, callback) => {
						...
						return Async.sortBy(responseObject.messages, (message, callback) => {
							...
							responseObject.messages = result.slice(0, clonedParams.limit);
		...
		return nylas.messages.list(nylasParams).then((response) => {
			...
			if (!clonedParams.includeDrafts) {
				responseObject.messages = response.filter((message) => {
					...
					return message.folder && message.folder.name !== 'drafts';
				});
