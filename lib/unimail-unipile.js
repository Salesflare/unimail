 ...
 * @typedef {import('./index').FolderResource} FolderResource
 */
...
class UnipileConnector extends EventEmitter {
    ...
    getMessage(auth, params, options, callback) {
        ...
        return internals.client.email.getOne(params.id, { extra_params: { account_id: auth.access_token, include_headers: true } }).then((response) => {
    ...
    listMessages(auth, params, options, callback) {
        ...
        // If there's already a before filter, use that instead if that date is newer
        ...
        if (
            ...
            (!clonedParams.before || new Date(clonedParams.pageToken) > new Date(clonedParams.before))
        ) {
            clonedParams.before = clonedParams.pageToken;
        ...
        // Get the folders
        // We need this to get the ids of folders to exclude
        // Also, if a folder is specified, get the folder id and call the listMessages method for each folder separately
        // Multiple folders at the same time are not supported by the unipile API
        try {
            return internals.getFolders(auth, (err, folders) => {

                // Don't fail the call if the sent or inbox folder is not found
                if (err && !err.message.includes('folder not found')) {
                    return callback(err);
                }

                const folderRolesToExclude = ['trash'];

                if (!clonedParams.includeDrafts) {
                    folderRolesToExclude.push('drafts');
                }

                const foldersToExclude = folders.filter((folder) => folderRolesToExclude.includes(folder.role)).map((folder) => folder.provider_id);

                options.foldersToExclude = foldersToExclude;

                if (clonedParams.folder) {
                // Filter out the folder with the correct attribute
                    const unipileFolders = folders.filter((folder) => folder.role.includes(clonedParams.folder.toLowerCase()));
                    ...
                    // Create an array of promises instead of using Async.map
                    const folderPromises = unipileFolders.map((unipileFolder) => {
                        ...
                        return new Promise((resolve, reject) => {
                            ...
                            return this.listMessagesCall(auth, folderCallParams, options, (err, result) => {
                    ...
                    return Promise.all(folderPromises)
                        .then((results) => {
                            ...
                            let messages = results.flatMap((result) => result.messages);
                            // Sort messages by date descending
                            messages.sort((a, b) => new Date(b.date) - new Date(a.date));
                            // Apply the limit
                            const limit = options.originalAttachmentLimit || clonedParams.limit;
                            messages = messages.slice(0, limit);

                            if (messages.length === 0) {
                                return callback(null, { messages, next_page_token: null });
                            }

                            // Fix the next_page_token
                            let lastEmailDate;
                            ...
                            callback(null, { messages, next_page_token: lastEmailDate });
                        })
                        .catch((err) => {

                            callback(Boom.boomify(err, { statusCode: 500 }));
                        });
    ...
    getFolders(auth, callback) {
        ...
        return internals.getFolders(auth, callback);
    }
    ...
    listMessagesCall(auth, clonedParams, options, callback) {
        ...
        if (options.foldersToExclude) {
            unipileOptions.extra_params.exclude_folders = options.foldersToExclude;
        }
        ...
        const processResponse = (response) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                try {
                    ...
                    // If we need more items and have a cursor, make a nested call
                    if (response.cursor && items.length < limit) {
                        const nestedParams = { ...clonedParams, pageToken: response.cursor };
                        const nestedOptions = {
                            ...options,
                            nested: true,
                            nested_limit: limit - items.length
                        };

                        return this.listMessagesCall(auth, nestedParams, nestedOptions, (err, nestedResponse) => {

                            if (err) {
                                return reject(err);
                            }

                            items = [...items, ...(clonedParams.hasAttachment
                                ? nestedResponse.items.filter((message) => message.attachments.length > 0)
                            ...
                            if (items.length > (options.originalAttachmentLimit || clonedParams.limit)) {
                                const effectiveLimit = options.originalAttachmentLimit || clonedParams.limit;
                                const lastEmail = items[effectiveLimit - 1];
                                if (lastEmail) {
                                    lastEmailDate = lastEmail.date;
                                }
                        ...
                        // If we're in a nested call
                        return resolve({ items });
                    }
                    else {
                        let lastEmailDate;
                        if (response.cursor) {
                            // Transform pagination to use the last email date instead of the cursor
                            const lastEmail = items[items.length - 1];
                            if (lastEmail) {
                                lastEmailDate = lastEmail.date;
                            }
                        }
        ...
        try {
            return internals.client.email.getAll(input, unipileOptions)
                .then(processResponse)
                .then((response) => {

                    if (options.nested) {
                        return callback(null, { items: response.items });
                    }
    ...
    listFiles(auth, params, options, callback) {
        ...
        return this.listMessages(auth, clonedParams, options, (err, response) => {
            ...
            const files = response.messages.flatMap((message) => message.files || []);
    ...
    /**
     * Returns the list of folders for the Unipile account, transformed into the unified folder shape.
     *
     * @param {Object} auth
     * @param {String} auth.access_token - Unipile account id used as access token
     *
     * @param {Object} [params]
     *
     * @param {Object} [options]
     * @param {Boolean} [options.raw] - If true the raw Unipile folder items are returned instead of the unified shape
     *
     * @param {function(Error?, (Array.<FolderResource> | Array.<Object>)?):void} callback - Returns an array of unified folder resources when options.raw is falsy or the raw items when truthy
     *
     * @returns {void}
     */
    listFolders(auth, params, options, callback) {
        ...
        return internals.client.email.getAllFolders({ account_id: auth.access_token }).then((folderResponse) => {
            ...
            return callback(null, this._transformFolders(folders));
        }).catch((err) => {
    ...
    /**
     * Transforms an array of raw Unipile folders into the unified folder shape.
     *
     * @param {Array.<Object>} folders - Raw folders returned by the Unipile API
     * @returns {Array.<FolderResource>}
     */
    _transformFolders(folders) {

        return (folders || []).map((folder) => {

            const role = folder.role && internals.unipileRoles.has(folder.role.toLowerCase())
                ? folder.role.toLowerCase()
                : null;
            ...
            return {
                ...
                role,
                parent_id: null,
                total_count: typeof folder.total_count === 'number' ? folder.total_count : null,
                unread_count: typeof folder.unread_count === 'number' ? folder.unread_count : null,
    ...
    getFile(auth, params, options, callback) {
        ...
        if (!params || !params.id || !params.messageId) {
            return callback(new Error('Invalid configuration. Please refer to the documentation to get the required fields.'));
        }
        ...
        return this.getMessage(auth, { id: params.messageId }, {}, (err, message) => {
            ...
            if (!fileObject) {
                return callback(Boom.notFound('Attachment not found'));
            }
            ...
            return internals.client.email.getEmailAttachment.byProviderId({
                ...
                .then((buffer) => {
                    // Convert the file data to Unimail format
                    fileObject.data = Buffer.from(buffer).toString('base64');
    ...
    _transformMessages(messagesArray) {
        ...
        return messages.map((message) => {
            ...
            const formattedMessage = {
                ...
                service_message_id: message.id || message.email_id,
                ...
                in_reply_to: message.in_reply_to?.message_id || message.in_reply_to,
                service_type: this.name,
                headers: internals.normalizeHeaders(message.headers || []),
                files
            };
    ...
    _transformFiles(filesArray) {
        ...
        return files.map((file) => {
            ...
            const formattedFile = {
                ...
                file_name: (!file.metadata.name || file.metadata.name === 'unknown') ? '' : file.metadata.name,
                ...
                service_type: this.name,
                is_embedded: file.metadata.isInline ? file.metadata.isInline : false,
            ...
            if (file.message.body.includes(`cid:${formattedFile.content_id}`) || file.metadata.cid) {
...
internals.unipileRoles = new Set(['inbox', 'sent', 'drafts', 'trash', 'spam', 'archive', 'important', 'starred', 'outbox', 'all']);
...
/**
 * Normalizes an array of { name, value } headers to an object where keys are
 * lowercased header names and values are arrays of header values.
 *
 * @param {Array.<{ name: String, value: String }>} headers
 * @returns {Object.<String, Array.<String>>}
 */
internals.normalizeHeaders = (headers) => {
    ...
    for (const header of headers) {
        ...
        normalizedHeaders[headerName].push(headerValue);
    }
...
internals.getFolders = (auth, callback) => {
    ...
    return internals.client.email.getAllFolders({ account_id: auth.access_token }).then((folderResponse) => {
        ...
        if (!inboxFolders || inboxFolders.length === 0) {
            ...
            if (inboxes.length === 0) {
                errors.push(Boom.notFound('Inbox folder not found'));
            }
            else {
                let inbox;
                if (inboxes.length > 1) {
        ...
        if (!sentFolders || sentFolders.length === 0) {
            ...
            if (sentFoldersGuess.length === 0) {
                errors.push(Boom.notFound('Sent folder not found'));
            }
            else {
                let sent;
        ...
        // We still want to return the folders, even if there are errors, so we can use them to filter the messages
        return callback(errors.length > 0 ? errors[0] : null, folders);
    }).catch((err) => {
