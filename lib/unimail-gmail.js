        ...
        return messages.map((message) => {
            ...
            nonContainerParts.forEach((nonContainerPart) => {
                ...
                if (nonContainerPart.filename && nonContainerPart.headers) {
                    ...
                    let contentId = internals.getHeaderValue(nonContainerPart.headers, 'content-id');
                    ...
                    formattedMessage.files.push({
                        ...
                        content_id: contentId,
                        content_disposition: contentDisposition,
                        service_file_id: nonContainerPart.partId,
                        is_embedded: contentDisposition ? contentDisposition.startsWith('inline;') : false
                    });
                }
            });
...
internals.parseEmailToAndFromHeaders = (recipientString) => {
    ...
    while ((m = regex.exec(recipientString)) !== null) {
        ...
        if (m[2]) {
            name = m[2].trim().replace(/,$/, '').replace(/"/g, "").trim(); // Strip whitespaces and commas, and remove quotation marks
        }

        if (m[3]) {
            email = m[3].trim().replace(/,$/, '').trim().toLowerCase(); // Strip whitespaces and commas from end of string, emails are always lower cased
        }
