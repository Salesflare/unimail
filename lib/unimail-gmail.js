        ...
        return Async.map(messages, (message, callback) => {
            ...
            return Utils.parseRawMail(message.payload.headers.map(h => `${h.name.toLowerCase()}: ${h.value}`).join('\n'), (err, mail) => {
                ...
                const formattedMessage = {
                    service_type: this.name,
                    email_message_id: mail.headers['message-id'][0] || null,
                    service_message_id: message.id,
                    service_thread_id: message.threadId,
                    date: mail.date,
                    subject: mail.headers['subject'][0] || null,
                ...
                if (mail.headers['in-reply-to'] && mail.headers['in-reply-to'].length > 0) {
                    formattedMessage.in_reply_to = mail.headers['in-reply-to'][0];
                }
                else {
                    formattedMessage.in_reply_to = null;
                }
