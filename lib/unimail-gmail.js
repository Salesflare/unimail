...
class GmailConnector extends EventEmitter {
        ...
        return this._callAPI(Gmail.users.messages.get.bind(Gmail.users.messages), gmailParams, (err, response) => {
            ...
            return this._transformMessages(response, (err, transformedMessages) => {
                ...
                if (transformedMessages.length === 0) {
                    return callback(Boom.notFound('Requested message not found'));
                }
    ...
    _processMessages(messages, options, nextPageToken, paramsArray, token, callback) {
        ...
        const processChunk = (chunk, cb) => {
            ...
            // Guard against Batchelor calling back multiple times (e.g. late socket error after response is already fully received)
            let cbCalled = false;
            ...
            batch.run((err) => {
                ...
                return Async.map(successfulParts, (part, mapCb) => {
                    ...
                    allTransformedMessages = [...allTransformedMessages, ...results].filter((message) => !!message);
                    return cb(null);
                });
            });
        };
        ...
        return Async.eachSeries(_.chunk(messages, 20), (chunk, seriesCb) => {
            ...
            if (errorBatch._requests && errorBatch._requests.length > 0) {
                ...
                errorBatch.run((err, retryResponses) => {
                    ...
                    const erroredParts = retryResponses.parts.filter((part) => {

                        return Number.parseInt(part.statusCode) > 399;
                    });

                    if (erroredParts.length > 0) {
                        const statusCode = erroredParts[0].statusCode ? Number(erroredParts[0].statusCode) : 500;
                        const errorMessage = statusCode === 429 ? 'Too many parts errored' : erroredParts[0].body?.error?.message;

                        return callback(Boom.boomify(new Error(errorMessage), { statusCode }));
                    }

                    // Process retry responses similarly
                    return Async.map(retryResponses.parts, (response, mapCb) => {
                        ...
                        allTransformedMessages = [...allTransformedMessages, ...retryResults].filter((message) => !!message);
    ...
    /**
     * Returns the list of Gmail labels for the authenticated user, transformed into the unified folder shape.
     *
     * @param {Auth} auth
     *
     * @param {Object} [params]
     *
     * @param {Object} [options]
     * @param {Boolean} [options.raw] - If true the raw labels array from the Gmail API is returned instead of the unified shape
     *
     * @param {function(Error?, (Array.<FolderResource> | Array.<Object>)?):void} callback - Returns an array of unified folder resources when options.raw is falsy or the raw labels array when truthy
     *
     * @returns {void}
     */
    listFolders(auth, params, options, callback) {
        ...
        return this._callAPI(Gmail.users.labels.list.bind(Gmail.users.labels), gmailParams, (err, response) => {
            ...
            return callback(null, this._transformFolders(labels));
        });
    }

    /**
     * Transforms an array of raw Gmail labels into the unified folder shape.
     *
     * @param {Array.<Object>} labels - Raw labels returned by the Gmail API
     * @returns {Array.<FolderResource>}
     */
    _transformFolders(labels) {
        ...
        return (labels || []).map((label) => {
            ...
            return {
                id: label.id,
                name: label.name,
                role: internals.labelIdToRole[label.id] || null,
                parent_id: null,
                total_count: typeof label.messagesTotal === 'number' ? label.messagesTotal : null,
                unread_count: typeof label.messagesUnread === 'number' ? label.messagesUnread : null,
    ...
    _transformMessages(messagesArray, callback) {
        ...
        let messages = (Array.isArray(messagesArray) ? messagesArray : [messagesArray]);
        messages = messages.filter((message) => message.payload && message.payload.headers); // Filter out messages that have no headers, these messages seem to contain barely any parseable content
