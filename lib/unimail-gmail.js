...
class GmailConnector extends EventEmitter {
        ...
        messages.forEach((message) => {
            ...
            nonContainerParts.forEach((part) => {
                ...
                files.push({
                    ...
                    email_message_id: message.headers.get('message-id') || null,
                    date: message.date,
    ...
    _transformMessages(messagesArray, callback) {
        ...
        return Async.map(messages, (message, callback) => {
            ...
            // Lowercase the header name to make sure we don't get derpes due to Message-Id vs message-id
            return SimpleParser(message.payload.headers.map(h => `${h.name.toLowerCase()}: ${h.value}`).join('\n'), (err, mail) => {
                ...
                const formattedMessage = {
                    service_type: this.name,
                    email_message_id: mail.headers.get('message-id') || null,
                    service_message_id: message.id,
                    service_thread_id: message.threadId,
                    date: mail.date,
                    subject: mail.headers.get('subject') || null,
                    folders: message.labelIds,
                    files: [],
                    body: [],
                    addresses: internals.getAddressesObject(mail),
                    in_reply_to: mail.headers.get('in-reply-to') || null,
...
internals.getAddressesObject = (mail) => {
    ...
    return {
        to: mail.to && mail.to.value.map(to => {
            return {
                name: to.name,
                email: to.address && to.address.toLowerCase()
            }
        }),
        from: mail.from && mail.from.value.map(from => {
            return {
                name: from.name,
                email: from.address && from.address.toLowerCase()
            }
        })[0] || {},
        cc: mail.cc && mail.cc.value.map(cc => {
            return {
                name: cc.name,
                email: cc.address && cc.address.toLowerCase()
            }
        }),
        bcc: mail.bcc && mail.bcc.value.map(bcc => {
            return {
                name: bcc.name,
                email: bcc.address && bcc.address.toLowerCase()
            }
        }),
    };
};
