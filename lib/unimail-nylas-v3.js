...
internals.getFolders = (auth, callback) => {
    ...
    return internals.nylas.folders.list({
        ...
        if (!inboxFolder) {
            const inboxes = folders.filter((folder) => folder.name.replace('INBOX.', '').toLowerCase().includes('inbox'));
            if (inboxes.length === 0) {
        ...
        if (!sentFolder) {
            const sentFolders = folders.filter((folder) => folder.name.replace('INBOX.', '').toLowerCase().includes('sent'));
