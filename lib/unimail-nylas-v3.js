...
class NylasV3Connector extends EventEmitter {
        ...
        if (clonedParams.folder) {
            try {
                return internals.nylas.folders.list({
                    ...
                    const folder = response.data.find((folder) => folder.attributes.includes(internals.folderMap[clonedParams.folder]));
                    clonedParams.folder = folder.id;
    ...
    listMessagesCall(auth, clonedParams, options, callback) {
        ...
        const nylasParams = {
            ...
            received_after: clonedParams.before ? Math.floor(new Date(clonedParams.after) / 1000) : undefined,
            to: clonedParams.to,
        ...
        const definedParams = Object.entries(nylasParams).reduce((acc, [key, value]) => {
            if (value !== undefined) {
              acc[key] = value;
            }
            return acc;
        ...
        try {
            return internals.nylas.messages.list({
                ...
                if (response.next_cursor) {
                    responseObject.next_page_token = response.next_cursor;
                }
    ...
    listFiles(auth, params, options, callback) {
        ...
        return this.listMessages(auth, params, options, (err, response) => {
            ...
            return Async.reduce(response.messages, [], (files, message, callback) => {
                ...
                return callback(null, { files, next_page_token: response.next_cursor });
            });
        });
    }
            ...
            return internals.nylas.attachments.download({
                ...
                response.on('end', () => { 
                    ...
                    return internals.nylas.messages.find({
                        ...
                        fileObject.message = response.data;
