...
/**
 * Folder resource, represents a mail folder/label and its metadata.
 * This object structure should be returned by all custom connectors for folders.
 *
 * @global
 * @typedef {Object} FolderResource
 * @property {String} id - Service-specific folder id (usable with listMessages folder filters)
 * @property {String} name - Human-readable folder name
 * @property {String|null} role - Standardized role: 'inbox' | 'sent' | 'drafts' | 'trash' | 'spam' | 'archive' | 'important' | 'starred' | 'outbox' | null
 * @property {String|null} parent_id - Parent folder id (null for flat hierarchies or top-level folders)
 * @property {Number|null} total_count - Total messages if exposed by the provider, else null
 * @property {Number|null} unread_count - Unread messages if exposed by the provider, else null
 * @property {String} service_type - Service name, same as the connector name
 */
...
class Unimail {
    ...
    constructor() {
        ...
        this.messages = {
            ...
            transform: (connectorName, auth, params, options) => {

                return this.callMethod(connectorName, 'transformMessages', auth, params, options);
            }
        };
        ...
        this.folders = {
            getAll: (connectorName, auth, callback) => {

                return this.callMethod(connectorName, 'getFolders', auth, callback);
            },
            list: (connectorName, auth, params, options, callback) => {

                return this.callMethod(connectorName, 'listFolders', auth, params, options, callback);
            }
        };
    }
...
Unimail.UnipileConnector = require('./unimail-unipile.js');
