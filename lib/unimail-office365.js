...
class Office365Connector extends EventEmitter {
     ...
     * @param {function(Error, (MessageResource | Object | String | null))} callback
     *
     * @returns {undefined} Returns a unified message resource when options.raw is falsy or a raw response from the API when truthy.
            ...
            return client.api(`/me/messages/${params.id}`)
                ...
                .get((err, resMessage) => {
                    ...
                    return this._transformMessages(resMessage, (err, transformedMessages) => {
                        ...
                        return client.api(`/me/messages/${transformedMessage.service_message_id}/attachments`).select('microsoft.graph.fileAttachment/contentId', 'microsoft.graph.fileAttachment/contentType', 'id', 'size', 'name', 'lastModifiedDateTime', 'isInline').get((err, resFiles) => {
     ...
     * @param {Boolean} options.raw - If true the response will not be transformed to the unified object
     *
     * @param {function(Error, (MessageListResource | MessageListResource[]))} callback
     *
     * @returns {undefined} - Returns an array of unified message resources when options.raw is falsy, or the raw response of the API when truthy
     */
                    ...
                    if (!params.includeDrafts) {
                        ...
                        if (!resMessages['@odata.nextLink']) {
                            ...
                            return this.listMessages(auth, recursiveParams, options, (err, recursiveResponse) => {
                                ...
                                return this._transformMessages(noDraftsList, (err, transformedMessages) => {
    ...
    sendMessage(auth, params, options, callback) {
        ...
        return Async.parallel({
            refreshToken: Async.apply(this._refreshTokenNeeded.bind(this), auth),
            ...
            convertAttachments: (callback) => {
                ...
                const convertToOffice365Attachment = (attachment, callback) => {

                    if (attachment.contentBytes) {
                        return callback(null, { ...attachment, '@odata.type': '#Microsoft.OutlookServices.FileAttachment' });
                    }
                ...
                return Async.map(params.attachments, convertToOffice365Attachment, (err, convertedAttachments) => {
            ...
            // Create a draft first to add the exension, then send it
            return client.api('/me/messages').post(message, (err, result) => {

                if (err) {
                    return callback(internals.wrapError(err));
                }

                return client.api(`/me/messages/${result.id}/send`).post({}, (err) => {

                    if (err) {
                        return callback(internals.wrapError(err));
                    }

                    return internals.attemptSentEmailFetch(client, params, hiddenUnimailGuid, 0, callback);
                });
            });
        });
    }
     ...
     * @param {Object} params
     * @param {Object} options
     * @param {function(Error, FilesResponse)} callback
     *
     * @returns {undefined}
     */
     ...
     * @param {{}} params
     * @param {String} uri
     * @param {function(Error, messages)} callback
     * @returns {undefined}
     */
    _getMessages(client, params, uri, callback) {
        ...
        return this._get(client, uri, callback);
    }
     ...
     * @param {String} authObject.refresh_token
     * @param {String} authObject.access_token
     * @param {String} authObject.expiration_date - The expiration date for the access token related to the refresh token.
     *
     * @param {function(Error, result)} callback
     *
 ...
 */
internals.getClient = (accessToken, version) => {
...
internals.wrapError = (errorObject) => {
    ...
    let statusCode = parseInt(errorObject.statusCode);
...
/**
 * @param { Array<Object> | Object } recipients
 * @param {Object} recipients.emailAddress
 * @param {String} recipients.emailAddress.name
 * @param {String} recipient.emailAddress.address
 *
 * @returns {[{}]|{}}
 */
 ...
 *
 * @param {Number | function(Error, String)} tokenLength
 * @param {function(Error, String)} callback
 * @returns {undefined}
 */
...
internals.attemptSentEmailFetch = (client, params, hiddenUnimailGuid, nrAttempts, callback) => {
    ...
    return client.api(`/me/messages?$search="from:${params.from.email} AND ${params.to.map((t) => `to:${t.email}`).join(' AND ')} AND subject:${params.subject.replace(/&/g, '')}"`)
        ...
        .get((err, messages) => {
            ...
            if (messages && Array.isArray(messages.value)) {
                sentMail = messages.value.find((message) => {
                    ...
                    const salesflareExtension = message.extensions.find((extension) => extension.extensionName === 'Unimail.hiddenGuid' && extension.guid === hiddenUnimailGuid);

                    return salesflareExtension;
                });
            }
