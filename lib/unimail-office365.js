 ...
 * It gets tried three times when no result is found before returning nothing
 *
 * @param {MicrosoftGraph.Client} client
...
internals.attemptSentEmailFetch = (client, params, hiddenUnimailGuid, nrAttempts, callback) => {
    ...
    /**
     * We remove the & of the subject search since the graph api considers it a special char and not part of the search string.
     * We also need to encode the search string since the version of graph sdk we use atm doesn't do it properly.
     */
    client.api(`/me/messages?$search="from:${params.from.email} AND ${params.to.map((t) => `to:${t.email}`).join(' AND ')} AND subject:${encodeURIComponent(params.subject.replace(/&/g, ''))}"`)
