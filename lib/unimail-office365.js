                        ...
                        if (!resMessages['@odata.nextLink']) {
                            ...
                            return this.listMessages(auth, recursiveParams, options, (err, recursiveResponse) => {
                                ...
                                if (options.raw) {
                                    ...
                                    return callback(null, internals.mapListOnlyResponse([...resMessages, ...recursiveResponse.messages]));
                                }
                        ...
                        if (!nextPageToken) {
                            ...
                            return this.listMessages(auth, recursiveParams, options, (err, recursiveResponse) => {
                                ...
                                if (options.raw) {
                                    ...
                                    return callback(null, internals.mapListOnlyResponse([...resMessages, ...recursiveResponse.messages]).messages);
                                }
    ...
    sendMessage(auth, params, options, callback) {
        ...
        return Async.parallel({
            ...
            if (attachments && attachments.length > 0) {
                ...
                if (attachments.some((attachment) => attachment['@odata.type'] === '#Microsoft.OutlookServices.ReferenceAttachment')) {
                    version = 'beta';
                }
            }
            ...
            .then((resFolders) => {
                ...
                return Async.each(requestFolders, (folderId, callback) => {
                    ...
                    client.api(folderUri).get().then((resMessages) => {
                        ...
                        allMessages = [...allMessages, ...resMessages.value];
...
internals.createFilterQuery = (params) => {
    ...
    if (params && !(Object.keys(params).length === 0 && params.constructor === Object)) {
        ...
        if (params.from) {
            ...
            const encodedParam = internals.encodeFilterParam(params.from);
        ...
        if (params.subject) {
            ...
            const encodedParam = internals.encodeFilterParam(params.subject);
...
/**
 * The Graph API splits url by '&' without taking quoted text in account, parameters would get split up and cause a badData to be returned.
 ...
 * @returns {String}
 */
internals.encodeFilterParam = (param) => {

    return param.replace(/&/g, '%26').replace(/'/g, '\'\'');
};
