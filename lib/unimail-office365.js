...
class Office365Connector extends EventEmitter {
    ...
    sendMessage(auth, params, options, callback) {
        ...
        return this._refreshTokenNeeded(auth, (err, token) => {
             ...
             */
            return internals.generateToken((err, hiddenSalesflareGuid) => {
                    ...
                    extensions: [
                        {
                            ...
                            extensionName: 'Salesflare.hiddenGuid',
                            guid: hiddenSalesflareGuid
                        }
                    ]
                };
                ...
                if (params.attachments && params.attachments.length > 0) {
                    ...
                    if (params.attachments.find(attachment => attachment['@odata.type'] === '#Microsoft.OutlookServices.ReferenceAttachment')) {
                        version = 'beta';
                    }
                }

                const client = internals.getClient(token.access_token, version);
                ...
                return client.api(`/me/messages`).post(message, (err, result) => {
                    ...
                    return client.api(`/me/messages/${result.id}/send`).post({}, (err) => {
                        ...
                        return internals.attemptSentEmailFetch(client, params, hiddenSalesflareGuid, 0, callback);
                    });
                });
            });
        });
 ...
 */
internals.getClient = (access_token, version) => {
 ...
 * @param {String} hiddenSalesflareGuid
 * @param {Number} nrAttempts
 * @param {function(Error, String)} callback
 * @returns {undefined}
 */
internals.attemptSentEmailFetch = (client, params, hiddenSalesflareGuid, nrAttempts, callback) => {
    ...
    return client.api(`/me/messages?$search="from:${params.from.email} AND ${params.to.map((t) => `to:${t.email}`).join(' AND ')} AND subject:${params.subject.replace(/&/g, '')}"`)
        .expand(`extensions($filter=id eq 'Salesflare.hiddenGuid')`)
        .get((err, messages) => {
            ...
            if (messages && Array.isArray(messages.value)) {
                sentMail = messages.value.find((message) => {
                    ...
                    const salesflareExtension = message.extensions.find((extension) => extension.extensionName === 'Salesflare.hiddenGuid' && extension.guid === hiddenSalesflareGuid);
            ...
            if (!(sentMail && sentMail.internetMessageId) && nrAttempts < 3) {
                return internals.attemptSentEmailFetch(client, params, hiddenSalesflareGuid, nrAttempts, callback);
            }
