...
class Office365Connector extends EventEmitter {
     ...
     * @param {function(Error, (MessageResource | Object | String | null)):void} callback
     *
     * @returns {void}
     */
     ...
     * @param {Boolean} [options.raw] - If true the response will not be transformed to the unified object
     *
     * @param {function(Error, (MessageListResource | MessageListResource[])):void} callback
     *
     * @returns {void} - Returns an array of unified message resources when options.raw is falsy, or the raw response of the API when truthy
     */
    ...
    sendMessage(auth, params, options, callback) {
        ...
        return Async.parallel({
            ...
            const apiRoute = params.inReplyTo ? client.api(`/me/messages/${params.inReplyTo}/reply`) : client.api('/me/messages');

            // Create a draft first to add the extension, then send it
            return apiRoute.post(message, (err, result) => {

                if (err) {
                    // This most likely means we are replying to an email not in our mailbox, if so retry the request without threadId
                    if (params.inReplyTo && err.statusCode === 400) {
                        return client.api('/me/messages').post(message, (err, resultWithoutReply) => {

                            if (err) {
                                return callback(internals.wrapError(err));
                            }

                            return client.api(`/me/messages/${resultWithoutReply.id}/send`).post({}, (err) => {
     ...
     * @param {Object} auth
     * @param {Object} params
     * @param {function(Error, file):void} callback
     *
     * @returns {void}
     */
     ...
     * @param {{}} params
     * @param {String} uri
     * @param {function(Error, (messages | null)):void} callback
     * @returns {undefined}
     */
