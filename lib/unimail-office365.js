...
internals.createFilterQuery = (params) => {
    ...
    if (params && !(Object.keys(params).length === 0 && params.constructor === Object)) {
        // We need to make sure `receivedDateTime` is the first thing in the filter query
        // This is a requisite from the Graph API to make sure we can order by `receivedDateTime`
        // https://docs.microsoft.com/en-us/graph/query-parameters#orderby-parameter
        if (!params.before && !params.after) {
            filterQuery += '?$filter=ReceivedDateTime gt 1970-00-00T00:00:00Z';
        }
        ...
        // The MS Graph API default sorting order is by `receivedDateTime` ascending, which we want to reverse
        filterQuery += `${filterQuery ? '&' : '?'}$orderby=ReceivedDateTime desc`;
