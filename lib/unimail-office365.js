...
class Office365Connector extends EventEmitter {
    ...
    listFiles(auth, params, options, callback) {
        ...
        /**
         * TODO: Something someting page tokens
         */
            ...
            try {
                client.api(`/me/messages${uri}`)
                    ...
                    .get((err, resMessages) => {
                        ...
                        messageIds = resMessages.value;
                        // resMessages.value.forEach(email => {
                        //     messageIds.push(email);
                        // });

                        for(let i = 0; i < messageIds.length; ++i) {
                            client.api(`/me/messages/${messageIds[i].id}/attachments`)
                            .get((err, resFiles) => {
                                ...
                                for (let iterator = 0; iterator < resFiles.value.length; ++iterator){
                                    resFiles.value[iterator].messageInfo = messageIds[i];
                                }
    ...
    getFile(auth, params, callback) {
        ...
        let asyncTasks = [];
        asyncTasks.push(function getMessage (client, params, callback){

            try {
                client.api(`me/messages/${params.messageId}`)
                    .select('id', 'from', 'toRecipients', 'ccRecipients', 'bccRecipients',
                    'sentDateTime', 'subject', 'internetMessageId', 'conversationId')
                    .get((err, resMessage) => {
        ...
        asyncTasks.push(function getFile (client, uri, callback){
        ...
        const asyncCallback = function asyncCallback (err, result) {

            if (result['getFile'] && result['getMessage']) {
                // fill file
                let file = result['getFile'];
                const message = result['getMessage'];

                file.messageInfo = message;

                return callback(null, this.transformFiles(file))
            }
            return callback(new Error('The file could not be found.'));
        }

        Async.parallel({
            getMessage: asyncTasks[0].bind(null, client, params),
            getFile: asyncTasks[1].bind(null, client, uri)
        ...
        try{
            client.api(uri)
                .get((err, res, rawResponse) => {
                    ...
                    res.headers = rawResponse.headers
                    return callback(null, res);
                });
        } catch (err) {
            return callback(err);
...
internals.transformMessage = (message) => {
    ...
    transformedMessage.service_message_id = message.id;
    transformedMessage.service_thread_id = message.conversationId;
    transformedMessage.email_message_id = message.internetMessageId;
    transformedMessage.subject = message.subject;
    transformedMessage.headers = message.headers;
    ...
    transformedMessage.folders = undefined;
    transformedMessage.attachments = message.hasAttachments;
    transformedMessage.service_type = 'office365';
...
internals.transformFile = (file) => {
    ...
    transformedFile.service_file_id = file.id;
    ...
    transformedFile.addresses = {
        from: file.messageInfo.from,
        to: file.messageInfo.toRecipients,
        cc: file.messageInfo.ccRecipients,
        bcc: file.messageInfo.bccRecipients 
    };
    transformedFile.date = file.messageInfo.sentDateTime;
    transformedFile.service_type = 'office365';
